...
class FisherCentralizedImplementation:
    def __init__(self, utilities,THRESHOLD):
        self.NCLO = 0
        self.counter = 0
        self.change = 0
    ...
    def calculate_x_ij(self):
        ...
        for i in range(self.nofAgents):
            for j in range(self.nofGoods):
                mission_id = self.utilities_[i][j].mission_entity.mission_id
                if self.prices[mission_id] != 0:
                    if self.utilities_[i][j].xij is None:
                        self.change = 9999999
                    else:
                        self.change += abs(((self.bids[i][j] / self.prices[mission_id]) - self.utilities_[i][j].xij))
                    if self.bids[i][j] / self.prices[mission_id] > 1E-10:
                        self.utilities_[i][j].xij = self.bids[i][j] / self.prices[mission_id]
                        self.NCLO = self.NCLO + 1
    ...
    def calculate_prices_initial(self):
        for j in range(self.nofGoods):
            mission_id = self.utilities_[0][j].mission_entity.mission_id
            self.prices[mission_id] = 0
            #self.prices[j] = 0
            for i in range(self.nofAgents):
                self.prices[mission_id]= self.prices[mission_id]+ self.bids[i][j]
    ...
    def print_xij(self):
        ...
        for i in range(self.nofAgents):
            ...
            for j in range(self.nofGoods):
                if j == 0:
                    print("Agent id_", self.utilities_[i][j].player_entity.id_, end=":  ")
...
class FisherCentralizedPrice ():
    def __init__(self, THRESHOLD, future_utility_function, tasks_simulation, players_simulation):
    ...
    def get_price_vector(self):
        ...
        fisher  = FisherCentralizedImplementation(util_matrix,self.THRESHOLD)
        fisher.algorithm()
        return fisher.prices

    def reset_dict(self):
        self.dict_ptmu = {}
        for player in self.players_simulation:
            self.dict_ptmu[player] = {}

            for task in self.tasks_simulation:
                self.dict_ptmu[player][task] = {}
                for mission in task.missions_list:
                    util = Utility(player_entity=player, mission_entity=mission, task_entity=task,
                                   t_now=0,  future_utility_function=self.future_utility_function)
    ...
    def turn_ptmu_to_util_matrix(self):
        ...
        for player, dict_task_dict in self.dict_ptmu.items():
            ...
            for task,dict_mission_util in dict_task_dict.items():
                for mission,util in dict_mission_util.items():
