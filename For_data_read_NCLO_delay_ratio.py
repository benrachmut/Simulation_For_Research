...
for folder_name in folder_names:
    ...
    for folder_of_excel_file1 in folders_of_excel_files1:
        ...
        amount_delay_ratio = create_amount_delay_lines_dict(unique_num_runs,df,field_name=field_name)


        comm = folder_name
        algo = folder_of_excel_file1
        line = [comm , algo ,amount_delay_ratio]
        lines.append(line)
        print(folder_of_excel_file1)
    df = pd.DataFrame(lines, columns=['Protocol', 'Algorithm', "Average"])
    #df = df.drop(0)
